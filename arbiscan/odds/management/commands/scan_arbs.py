...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--clear",          action="store_true",
                            help="Kept for old scripts; every scan now replaces previous results")
    ...
    def handle(self, *args, **options):
        ...
        qs = Fixture.objects.filter(status="upcoming", kickoff__gt=timezone.now()).prefetch_related(
            ...
            if not nm_only:
                for r in arbs:
                    if r["profit_pct"] >= options["min_profit"]:
                        ...
                        new_rows.append(ArbitrageResult(
                            fixture=fixture, market=r["market"], books=r["books"],
                            pair_label=r["pair"], odds=r["odds"], stakes=r["stakes"],
                            bankroll=bankroll, payout=r["payout"], profit=r["profit_kes"],
                            profit_pct=r["profit_pct"], arb_sum=r["sum"],
                            is_high_odds=max_leg >= 4.0, is_valid=True))
            if nm_only and nms:
                best = nms[0]
                self.stdout.write(
        ...
        if not nm_only:
            # Cross-book results are derived data: replace the set so closed arbs
            # disappear. Intra-book rows (INTRA:*) belong to scan_intra_arbs.
            with transaction.atomic():
                cleared, _ = ArbitrageResult.objects.exclude(pair_label__startswith="INTRA:").delete()
                ArbitrageResult.objects.bulk_create(new_rows)
            self.stdout.write(f"Replaced {cleared} previous results")

        self.stdout.write(self.style.SUCCESS(
            f"\nDone - {len(new_rows)} arbs saved, {total_nm} near-misses."))
